...
net.Receive("gmstation_achievement", function()
    ...
    if IsValid(who) and who == LocalPlayer() then
...
if css:IsError() or tf2:IsError() then
    local games = ""
...
local last_commit_hash = file.Read("addons/gmstation/.git/refs/heads/master", "GAME") or "failed to read .git repo"
local sfile = file.Read("addons/gmstation/.superposed", "GAME") or "failed to read .superposed file"
